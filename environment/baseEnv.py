...
class BaseEnv:
    ...
    def disable_all_obstacles(self):
        for i, (obs_id, obs) in enumerate(self.all_obs.items()):
            obs.disable(idx=i)
        self.active_obs = {}

    def enable_obstacles(self, obs_list=[]):
        self.disable_all_obstacles()
        for id, obs in self.all_obs.items():
            if id in obs_list:
                obs.enable()
                self.active_obs[id] = obs
    ...
    def step(self, actions):
        ...
        try:
            actions_step = actions.norm()
        except:
            actions_step = max([action.norm() for action in actions])
        
        if actions_step < self.early_stop_threshold:
    ...
    def setup_task(self):
        ...
        self.enable_obstacles(obs_list=self.get_current_task().obstacles.keys())
        for id, obs in self.active_obs.items(): 
            obs.set_position(self.get_current_task().obstacles[id])
